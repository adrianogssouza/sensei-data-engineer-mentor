...
- Checkpoint atual - Concluído: v0.1-beta já protege tabelas Supabase com RLS validado, expõe health documental no overview e em documentos, persiste conteúdo bruto por texto colado ou arquivo textual, permite editar documentos, filtrar por status, reprocessar chunks individualmente/em lote, acompanhar fila de embeddings, buscar com ranking lexical, gerar embeddings mock em pgvector, usar recuperação híbrida no chat mock, exibir diagnóstico visual e rodar evals versionados.
