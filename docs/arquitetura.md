...
O shell do workspace foi implementado como navegação e páginas placeholder. O chat foi implementado como UI/estado com persistência em `localStorage`. A TASK 009 adicionou o skeleton de provider de IA e a TASK 010 adicionou Gemini como primeiro provider real. O registry de providers suporta mock e Gemini, com mock sempre disponível e Gemini selecionado apenas quando configurado explicitamente via variáveis de ambiente. `/workspace/chat` chama `/api/ai/chat`, que valida um payload mínimo de mensagens e usa a abstração de provider. Não há SDKs Anthropic/OpenAI instalados, leitura/escrita no Supabase, RAG, embeddings, upload, pgvector, streaming, persistência de uso ou persistência de chat em cloud.
