...
Os componentes planejados acima ainda não foram implementados. O estado atual do repositório já possui documentos manuais, `raw_content`, `document_chunks` e busca lexical/local ranqueada, mas ainda não possui extensão pgvector, coluna de embeddings, SDK Anthropic, SDK OpenAI, upload/parsing de arquivos, pipeline RAG semântico, eval runner, ownership multi-user com `user_id`, política RLS final, persistência de uso em banco ou streaming.
