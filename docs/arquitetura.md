...
- Eval manual de recuperação em `/workspace/documents` e `/api/documents/retrieval-evals`.
...
Os componentes planejados acima ainda não foram implementados. O estado atual do repositório já possui documentos manuais, `raw_content`, `document_chunks`, busca lexical/local ranqueada, extensão pgvector, colunas de embedding, geração mock de embeddings, recuperação híbrida lexical + vetorial, diagnóstico visual de recuperação no chat e eval manual de recuperação, mas ainda não possui embeddings reais por provider externo, SDK Anthropic, SDK OpenAI, upload/parsing de arquivos, dataset versionado de evals, pipeline RAG semântico, ownership multi-user com `user_id`, política RLS final, persistência de uso em banco ou streaming.
